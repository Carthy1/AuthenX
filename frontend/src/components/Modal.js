...
function Modal({ type, close }) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        status: isSuperAdmin ? "active" : "pending_gtec",
      ...
      toast.success("Application Submitted! Under GTEC Accreditation Review.");
      setMode("login");
    } catch (error) {
      clearTimeout(safetyTimeout);
  ...
  const handleLogin = async () => {
    ...
    if (email === "gtec@demo.com") {
      finalizeLogin({
        uid: "gtec123",
        email: "gtec@demo.com",
        fullName: "GTEC Auditor",
        institution: "Ghana Tertiary Education Commission",
        role: "gtec"
      });
      setLoading(false);
      return;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn("Firestore fetch failed (e.g., missing permissions):", fallbackError);
          userData = null;
        }
      }
